...
import { useListOrders, useMarkOrderPaid, useUpdateOrder } from "@workspace/api-client-react";
...
import { Search, CheckCircle, Clock, Package, MapPin, CreditCard, Truck, Store, X, CalendarDays } from "lucide-react";
        ...
        <DialogContent className="bg-slate-900 border-slate-800 text-slate-100 max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {viewOrder && (
            <div className="space-y-6 mt-2">
              ...
              {sd && sd.appointmentDate && (
                <div className="mt-4 bg-amber-500/10 border border-amber-500/20 rounded-md p-4">
                  <h3 className="font-semibold text-sm text-amber-500 flex items-center gap-2 mb-2">
                    <CalendarDays className="h-4 w-4" /> Requested Appointment
                  </h3>
                  <div className="flex gap-6 text-sm text-slate-200">
                    <div><span className="text-slate-400">Date:</span> {sd.appointmentDate}</div>
                    <div><span className="text-slate-400">Time:</span> {sd.appointmentTime}</div>
                  </div>
                </div>
              )}
